...
if($language == '' && $_POST['submit'] == ''){
	$url = 'http://www.pligg.com/languages/check/getLanguageList.php?type=upgrade&version=120';
	...
	if($language != 'local'){
	    $url = 'http://www.pligg.com/languages/check/getLanguageFile.php?type=installer&version=120&language=' . $language;
	    $r = new CD_HTTPRequest($url);
	    $data = $r->DownloadToString();
...
// echo $languageContent;
	...
	if (!$tableexists) {
		...
		$db->query("ALTER TABLE `pligg_widgets` ADD UNIQUE (`folder`)");
	...
	if (!$tableexists) {
		...
		$sql = "INSERT INTO `" . table_misc_data . "` ( `name` , `data` ) VALUES ('pligg_version', '1.2.0');";
		$db->query($sql);
		//Captcha upgrade:
		...
		$sql = "UPDATE `" . table_misc_data . "` SET `data` = '1.2.0' WHERE `name` = 'pligg_version';";
		$db->query($sql);
        	//Captcha upgrade:
